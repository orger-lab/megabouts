...
class BoutClassifier:
    ...
    def extract_bouts(self, **kwargs):
        if self.tracking_cfg.tracking == "full_tracking":
            ...
            raise ValueError(f"No implemented yet tracking mode: {self.tracking_cfg.tracking}")
    ...
    def compute_sampling_input(self, num_samples):
        ...
        t_ = np.linspace(0, self.segmentation_cfg.bout_duration_ms, self.segmentation_cfg.bout_duration, endpoint=False)
    ...
    def process_results(self, results, num_samples):
        ...
        for result in results:
            ...
            logit_label, logit_sublabel, logit_bout_sign, t_p = [x.cpu().detach().numpy() for x in result]
            logit_label,logit_sublabel = self.filter_logit(logit_label,logit_sublabel)
            proba[id_batch] = np.max(np.exp(logit_label) / np.sum(np.exp(logit_label), axis=1, keepdims=True), axis=1)
