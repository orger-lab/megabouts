...
class ContinuousPositionalEncoding(nn.Module):
    def __init__(self, d_model, max_seq_length):
        ...
        denominators = torch.pow(10000, 2*torch.arange(0, self.d//2)/self.d) # 10000^(2i/d_model), i is the index of embedding
...
class TransAm(nn.Module):
    def __init__(self,mapping_label_to_sublabel,feature_size=64,num_layers=3,dropout=0.,nhead=8):
        super(TransAm, self).__init__()
        self.model_type = 'Transformer'
        self.input_embedding  = nn.Linear(11,feature_size)
        self.pos_encoder = ContinuousPositionalEncoding(d_model=feature_size,max_seq_length=140)
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=feature_size, nhead=nhead, dropout=dropout,batch_first=True)
    ...
    def forward(self,input,t,mask):
        ...
        mask = torch.cat([torch.zeros((output.shape[0],1),dtype=torch.bool,device = mask.device), mask], dim=1)
        output = self.transformer_encoder(output,src_key_padding_mask=mask)#,self.src_mask)
        output_CLS = output[:,0,:]
        ...
        logit_label = torch.zeros((output_bout_cat.shape[0],13),dtype=output_bout_cat.dtype,device = output_bout_cat.device)
        for i in range(13):
            id = self.mapping_label_to_sublabel[i]
            logit_label[:,i] = torch.log(torch.sum(torch.exp(logit_sublabel[:,id]),axis=1))
