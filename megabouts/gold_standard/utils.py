...
def compute_edge_column(x,y,body_angle,arena_radius):
    radius = np.sqrt(x*x+y*y)
    distance2edge = arena_radius-radius
    radius_vect=np.vstack((x,y)) # Compute Radius Vector
    body_vect=np.vstack((np.cos(body_angle),np.sin(body_angle))) # Compute Body Vector
    angle2edge=compute_angle_between_vect(radius_vect.T,body_vect.T)
    return distance2edge,angle2edge

def refine_onset_offset(tail_angle_hat,bout_indicator_init,z_sum,Min_Bout_Duration):
    bout_indicator0 = 1.0*bout_indicator_init
    firstbeatsearch = 0.0*bout_indicator_init

    onset,offset,duration = find_onset_offset_numpy(bout_indicator0)
    onset,offset,duration = onset.tolist(),offset.tolist(),duration.tolist()

    # Correct Onset as first peak


    onset_corrected =[]
    for on_,off_,dur_ in zip(onset,offset,duration):
        margin_m = 20
        margin_p = min(120,max(dur_-30,Min_Bout_Duration))
        firstbeatsearch[on_-margin_m:on_+margin_p]=1
        i,bout_sign = find_bout_onset(tail_angle_hat[:,7],on_,margin_m,margin_p,MinInterPeakDistance = 10,Ratio_max=0.4)
        onset_corrected.append(int(on_-margin_m+i))

    onset = onset_corrected

    # Correct Offset as last spike:
    offset_corrected = []
    for on_,off_ in zip(onset,offset):
        if off_-on_>Min_Bout_Duration:
            id = np.where(np.abs(z_sum[on_:off_])>0)[0]
            off_ = on_+max(Min_Bout_Duration,id[-1])
        else:
            off_ = on_+Min_Bout_Duration
        offset_corrected.append(off_)

    #onset_corrected = onset
    #offset_corrected =offset

    bout_indicator1 = np.zeros_like(bout_indicator0)
    for on_,off_ in zip(onset_corrected,offset_corrected):
        bout_indicator1[on_:off_]=1
        
    return bout_indicator1
