...
class TrajPreprocessingResult:
    def __init__(self, x, y, yaw, 
                 x_smooth, y_smooth, yaw_smooth, 
                 axial_speed, lateral_speed, yaw_speed, 
                 vigor, no_tracking):
    ...
    def _to_dataframe(self):
        ...
        data = np.vstack((
            ...
            self.x_smooth, self.y_smooth, self.yaw_smooth,
            self.axial_speed, self.lateral_speed, self.yaw_speed,
            self.vigor, self.no_tracking
        )).T
...
class TrajPreprocessing():
    ...
    def preprocess_traj_df(self, traj_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        x_input,y_input,yaw_input = traj_df['x'].values,traj_df['y'].values,traj_df['yaw'].values
        
        x,y,yaw,no_tracking = TrajPreprocessing.interp_traj_nan(x_input,y_input,yaw_input,
                                                                limit_na=self.config.limit_na)    
        smooth_func = lambda x : TrajPreprocessing.one_euro_filter(x,
                                                                   self.config.freq_cutoff_min,
                                                                   self.config.beta,
                                                                   self.config.fps)
        x,y,yaw  = map(smooth_func,[x,y,yaw])

        axial_speed,lateral_speed,yaw_speed = TrajPreprocessing.compute_speed(x,y,yaw,
                                                                              self.config.fps,
                                                                              n_diff=self.config.robust_diff)
        vigor = TrajPreprocessing.compute_kinematic_activity(axial_speed,lateral_speed,yaw_speed,
                                                                          lag=self.config.lag_kinematic_activity,
                                                                          fps=self.config.fps)
        
        return TrajPreprocessingResult(x_input,y_input,yaw_input,
                                        x,y,yaw,
                                        axial_speed, lateral_speed, yaw_speed,
                                        vigor, no_tracking)
    ...
    @staticmethod
    def interp_traj_nan(x: np.ndarray, y: np.ndarray, yaw: np.ndarray, limit_na: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    @staticmethod
    def one_euro_filter(x: np.ndarray, freq_cutoff_min: float, beta: float, rate: int) -> np.ndarray:
    ...
    @staticmethod        
    def compute_speed(x: np.ndarray, y: np.ndarray, yaw: np.ndarray, fps: int, n_diff: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    @staticmethod
    def compute_kinematic_activity(axial_speed: np.ndarray, lateral_speed: np.ndarray, yaw_speed: np.ndarray, lag: int, fps: int) -> np.ndarray:
