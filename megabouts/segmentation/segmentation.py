...
class SegmentationResult():
    def __init__(self,config:SegmentationConfig, onset: np.ndarray, offset: np.ndarray,T:int):
        self.config=config
    ...
    def set_HB1(self, first_half_beat: np.ndarray):
        if len(first_half_beat) != len(self.onset):
            raise ValueError("Length of first_half_beat must be equal to the length of onset")
    ...
    def extract_tail_array(self,*,tail_angle: np.ndarray,align_to_onset:bool = True) -> np.ndarray:
        ...
        tail_array = np.zeros((len(onset), tail_angle.shape[1], self.config.bout_duration))
    ...
    def extract_traj_array(self,*,head_x: np.ndarray,
                           head_y: np.ndarray,
                           head_angle: np.ndarray,
                           align_to_onset: bool = True,
                           align: bool = True, 
                           idx_ref: int = 0) -> np.ndarray:
        ...
        for i, id_st in enumerate(onset):
            ...
            id_ed = min(id_ed,self.T-1)
            dur = id_ed-id_st
            sub_x, sub_y, sub_head_angle = x[id_st:id_ed], y[id_st:id_ed], head_angle[id_st:id_ed]
            traj_array[i, 0, :dur], traj_array[i, 1, :dur], traj_array[i, 2, :dur] = sub_x, sub_y, sub_head_angle
    ...
    def align_traj_array(self,traj_array: np.ndarray,
                         idx_ref) -> np.ndarray:
        
        traj_array_aligned = np.zeros_like(traj_array)
        N = traj_array.shape[0]
        for i in range(N):
            sub_x, sub_y, sub_head_angle = traj_array[i, 0, :], traj_array[i, 1, :], traj_array[i, 2, :]
            ...
            sub_x, sub_y, sub_head_angle = PosRot[0, :], PosRot[1, :], head_angle_rotated
            traj_array_aligned[i, 0, :], traj_array_aligned[i, 1, :], traj_array_aligned[i, 2, :] = sub_x, sub_y, sub_head_angle
...
class TrajSegmentation(Segmentation):
    ...
    def segment_from_traj(self, kinematic_activity: np.ndarray) -> SegmentationResult:
        ...
        peaks, _ = find_peaks(kinematic_activity, 
        ...
        onset, offset = TrajSegmentation.find_onset_offset_around_peak(kinematic_activity, peaks, inter_peak_min,
                                                                       self.config.peak_percentage)

        segments = SegmentationResult(self.config,onset,offset,len(kinematic_activity))
    ...
    def find_inter_peak_min(x, peaks):
        ...
        inter_peak_min = [p1 + np.argmin(x[p1:p2]) for p1, p2 in zip(peaks_list[:-1], peaks_list[1:])]
        return np.array(inter_peak_min)

    @staticmethod
    def find_onset_offset_around_peak(x: np.ndarray, peaks: np.ndarray, inter_peak_min: np.ndarray, peak_percentage: float) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for p_before, p, p_after in zip(inter_peak_min[:-1], peaks, inter_peak_min[1:]):
            # Find onset
            on_ = p
            while on_ > p_before and x[on_] >= .25 * x[p]:
                on_ -= 1
