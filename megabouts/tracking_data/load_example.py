...
def load_example_data(source):
    ...
    if source not in sources:
        raise ValueError("Invalid source provided. Available sources are: 'fulltracking_posture', 'SLEAP_fulltracking', 'HR_DLC', 'zebrabox_SLEAP'.")

    metadata = sources[source]
    csv_path = os.path.join(os.path.dirname(__file__),'example_dataset', metadata['file'])
    if source=='HR_DLC':
