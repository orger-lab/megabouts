...
import numpy as np
import pandas as pd
from megabouts.tracking_data.convert_tracking import compute_angles_from_keypoints
from megabouts.tracking_data.convert_tracking import convert_tail_angle_to_keypoints
from megabouts.tracking_data.convert_tracking import interpolate_tail_keypoint,interpolate_tail_angle
...
class FullTrackingData(TrackingData):
    ...
    def from_keypoints(cls,*,head_x,head_y,tail_x,tail_y):
        ...
        tail_angle, head_yaw = compute_angles_from_keypoints(head_x,head_y,tail_x,tail_y)
    ...
    @property
    def tail_df(self):
        tail_df = pd.DataFrame(self._tail_angle, columns=[f'angle_{i}' for i in range(10)])
        return tail_df
    ...
    def _validate_keypoints(head_x,head_y,tail_x,tail_y):
        ...
        if N_keypoints<4:
            raise ValueError('At least 4 points from swim bladder to tail tips are required for full tracking')
        if tail_x.shape[1] != tail_y.shape[1]:
            raise ValueError("tail_x and tail_y must have the same number of keypoints (N).")

    @staticmethod
    def _validate_posture(head_x,head_y,head_yaw,tail_angle):
        T = len(head_x)
        if not (len(head_y) == T and head_yaw.shape[0] == T and tail_angle.shape[0] == T):
            raise ValueError("All inputs must have the same number of time points (T).")
...
class HeadTrackingData(TrackingData):
    ...
    def from_keypoints(cls,*,head_x,head_y,swimbladder_x,swimbladder_y):
        ...
        tail_angle, head_yaw = compute_angles_from_keypoints(head_x,head_y,
                                                               swimbladder_x[:,np.newaxis],
                                                               swimbladder_y[:,np.newaxis])
    ...
    def _validate_keypoints(head_x,head_y,swimbladder_x,swimbladder_y):
        T = len(head_x)
        if not (len(head_y) == T and len(swimbladder_x) == T and len(swimbladder_y) == T):
            raise ValueError("All inputs must have the same number of time points (T).")
...
class TailTrackingData(TrackingData):
    ...
    def from_keypoints(cls,*,tail_x,tail_y):
        ...
        if tail_x.shape[1] != 11:
            tail_x, tail_y = interpolate_tail_keypoint(tail_x, tail_y, 10)
        tail_angle,head_yaw = compute_angles_from_keypoints(tail_x[:,0]+0.5,tail_y[:,0],tail_x,tail_y)
    ...
    def from_posture(cls,*,tail_angle):
        ...
        tail_x,tail_y = convert_tail_angle_to_keypoints(head_x, head_y, head_yaw, tail_angle, body_to_tail_mm=0., tail_to_tail_mm=0.32)
        return cls(tail_x,tail_y,tail_angle)
    ...
    def _validate_keypoints(tail_x,tail_y):
        ...
        N_keypoints = tail_x.shape[1]
        if N_keypoints<4:
            raise ValueError('At least 4 points from swim bladder to tail tips are required for tail tracking')
        if tail_x.shape[1] != tail_y.shape[1]:
            raise ValueError("tail_x and tail_y must have the same number of keypoints (N).")
