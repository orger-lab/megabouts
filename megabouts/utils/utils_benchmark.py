...
def compute_bout_cat_matlab(df,fps_new=700,fps_old=700):
    ...
    #downsampling_f, Duration_after_Downsampling,t,tnew = create_downsampling_function(fps_new=fps_new,fps_origin=fps_old,duration_ms=len(tail_active)*1000/700)
    downsampling_f, Duration_after_Downsampling,t,tnew =  create_downsampling_function(fps_new=fps_new,fps_origin=fps_old,duration=len(tail_active),duration_unit='frames',kind='linear')
